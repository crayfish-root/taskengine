...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!canApprove) return NextResponse.json({ error: "Not authorized to act on this request" }, { status: 403 });

  const updated = await prisma.leaveRequest.update({
    where: { id },
    data: { status: "REJECTED", approverId: currentUser.id },
  });
  ...
  await prisma.notification.create({
    data: {
      ...
      body: `Your leave request was rejected by ${currentUser.name}.`,
      link: "/leave",
    },
  });
